...
$(document).ready(function () {
  ...
  $("#name").keyup(function () {
    ...
    if (name == "") {
      ...
      if (length < minLength || length > maxLength) {
        $("#name").addClass("is-invalid");
        $("#showErrorName").text("Nhập giá trị từ 10 đến 500 ký tự!");
        checkName = 10;
      } else {
...
app.controller("blog-form-ctrl", function ($scope, $http) {
  ...
  $scope.update = function () {
    if (checkForm()) {
      ...
      $http
        .put("/rest/blogs/" + $("#demo").val(), item)
        .then((resp) => {
          $scope.info.status = true;
          $scope.info.content = "Bạn đã cập nhật Blog thành công!";
